#!/bin/bash
# Morse Trainer GUI Linux/macOS Startup Script

# Locate lame: prefer known paths, fall back to PATH
for p in /usr/bin/lame /usr/local/bin/lame /opt/homebrew/bin/lame; do
    if [ -x "$p" ]; then export MTLAME="$p"; break; fi
done

# Locate espeak/espeak-ng: prefer known paths, fall back to PATH
